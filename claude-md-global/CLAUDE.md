...
- Before you start coding, check the linter rules in "eslint.config.mjs" (or equivalent) file once per session. Do this without running the linter from the console. Try to understand what these rules mean (as much as possible), and write your code according to these rules. There is no need to run the linter from the command line (like "yarn lint") at the end.
...
- If Claude has doubts about a solution (security, performance, complexity issues), it should FIRST discuss concerns with me and explain why it might be a bad idea, rather than blindly implementing it. Provide technical reasoning and alternatives.
- Never commit on your own unless I explicitly ask you to.
